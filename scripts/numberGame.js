...
function verifyGuess() {
    ...
    if (userGuess === randomNumber) {
        ...
        lastResult.textContent= 'Doğru tapdınız! Təbriklər!';
        lastResult.style.color = 'yellow';
        lastResult.style.backgroundColor = 'green';
        
        lowOrHi.textContent = 'Uyğundur!';
        setGameOver();
      } else if (guessCount === 5) {
        lastResult.textContent = 'Oyun bitdi !!';
        setGameOver();
      } else {
        lastResult.textContent = 'Yanlışdır!';
        lastResult.style.backgroundColor = 'red';

        if(userGuess < randomNumber) {
            lowOrHi.textContent = 'Əvvəlki təxmin kiçikdir!!';
          } else if(userGuess > randomNumber) {
            lowOrHi.textContent = 'Əvvəlki təxmin böyükdür!';
          }
      }
  ...
  */
  function createResetButton() {
    ...
    resetButton.textContent = 'Yeni oyuna başlayın';
